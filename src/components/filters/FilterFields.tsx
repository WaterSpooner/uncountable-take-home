...
export function FilterFields({ fields, filters, onChange }: FilterFieldsProps) {
    ...
    return (
        <div>
            <span>
                <button className={styles.addFilterButton} type="button" onClick={() => setPartialFilters((prev) => [...prev, {id: crypto.randomUUID()}])}>New Filter</button>
            </span>
            <div className={styles.container}>
                {filters.map((filter) => (
                    <FilterComponent 
                        ...
                        filter={filter}
                        fields={fields}
                        updateFilter={(filter) => onChange(filters.map((f) => f.id === filter.id ? filter : f))}
                ...
                {partialFilters.map((filter) => (
                    <FilterComponent 
                        ...
                        fields={fields}
                        updateFilter={(filter) => {
                            setPartialFilters((prev) => prev.filter((f) => f.id !== filter.id));
