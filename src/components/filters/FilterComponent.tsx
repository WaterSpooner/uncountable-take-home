...
export function FilterComponent(props: FilterComponentProps) {
  ...
  const [fieldType, setFieldType] = useState<ExperimentFieldType | ''>(
    filter.fieldType ?? '',
  );
  const [field, setField] = useState<InputField | OutputField | ''>(
    filter.field ?? '',
  );
    ...
    <div className={styles.container}>
      <span className={styles.field}>
        ...
        <Dropdown
          ...
          options={Object.keys(fields) as ExperimentFieldType[]}
          selectedOption={fieldType}
          onSelect={(option) =>
            setFieldType(option as ExperimentFieldType | '')
          }
        />
      </span>
      <span className={styles.field}>
        ...
        <Dropdown
          displayText={'Field'}
          options={fieldType ? fields[fieldType] : []}
          selectedOption={field}
          onSelect={(option) => setField(option)}
        />
      </span>
      <span className={styles.field}>
        ...
        <Dropdown
          ...
          onSelect={(option) => setOperator(option)}
        />
      </span>
      <span className={styles.field}>
