...
export default function ScatterPlot({ dataset, xAxisOptions, yAxisOptions }: ScatterPlotProps) {

  const [xAxis, setXAxis] = useState<InputField | "">("");
  const [yAxis, setYAxis] = useState<OutputField[]>([]);

  const formattedData = useMemo(() => formatData(dataset, xAxis, yAxis), [dataset, xAxis, yAxis]);
  ...
  return (
    ...
    <ResponsiveContainer width="100%" height={600}>
      <ScatterChart >
        <CartesianGrid />
        <XAxis type="number" dataKey={xAxis} name={xAxis} domain={['dataMin', 'dataMax']}> 
          <Label value={xAxis} offset={0} position="insideBottom"/>
        </XAxis>
        {yAxis.map((yLabel) => (
          <YAxis type="number" dataKey={yLabel} yAxisId={yLabel} name={yLabel} key={yLabel}>
            <Label value={yLabel} offset={20} position="insideLeft" angle={-90} textAnchor="middle" />
          </YAxis>
        ))}
        {yAxis.map((yLabel) => (
          <Scatter
    ...
    <span>
      <div className={styles.controlsGrid}>
        ...
        <MultiSelector
          ...
          setSelectedInputs={(selected) => setYAxis(selected as OutputField[])}
        />
      </div>
      <div>
...
function ScatterTooltip({ active, payload }: ScatterTooltipProps) {
  ...
  return (
    <div className={styles.tooltipBox}>
      ...
      {payload.map((entry) => (
        <div key={String(entry.name)} style={{ color: entry.color ?? "#111827" }}>
