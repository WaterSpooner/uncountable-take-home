...
export function MultiSelector<T>(props: MultiSelectorProps<T>) {
    ...
    return (
        <div>
            ...
            <div>
                <div key={"Select All"} className={`${styles.itemContainer} experiment-item`}>
                    <input 
                        type="checkbox" 
                        id="Select All"
                        name="Select All"
                    ...
                    <label htmlFor="Select All" className="experiment-label">
                        <span className={`experiment-id ${styles.selectAllLabel}`}>Select All</span>
                    </label>
                </div>
                {searchedInputs.map((input) => (
                    <div key={String(input)} className={`${styles.itemContainer} experiment-item`}>
                        <input 
                        type="checkbox" 
                        id={String(input)} 
                        name={String(input)}
...
function searchInputs<T>(inputs: T[], search: string): T[] {
    return inputs.filter((input) => String(input).toLowerCase().includes(search.toLowerCase()));
}