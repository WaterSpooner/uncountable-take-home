  ...
  const filteredData = useMemo(() => {
    return Object.fromEntries(
      Object.entries(dataset)
        ...
        .filter(([experimentId, _]) =>
          selectedExperiments.includes(experimentId as ExperimentId),
        ),
    ) as Dataset;
...
function evaluateFilter(experiment: Experiment, filter: Filter) {
  ...
  if (filter.fieldType === 'inputs') {
    fieldValue =
      experiment.inputs[filter.field as keyof typeof experiment.inputs];
  } else if (filter.fieldType === 'outputs') {
    fieldValue =
      experiment.outputs[filter.field as keyof typeof experiment.outputs];
  } else {
    return false;
  }
